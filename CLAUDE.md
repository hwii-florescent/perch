...
- `boot.rs` — shared boot recipe used by both binaries (`boot()` with optional readiness oneshot reporting the bound addr; port 0 → `127.0.0.1:0` for the desktop). Also prepends `~/.local/bin` to PATH at startup so claude/codex are found under minimal-PATH launches (tmux, launchd, Tauri).
- `db.rs` (rusqlite, `~/.perch/history.sqlite`), `registry.rs` (replay ring buffer), `settings.rs`/`hosts.rs` (`~/.perch/*.json`, all-`serde(default)`, atomic temp+rename writes), `models.rs` (static model catalogue — deliberately not version-gated or probed per host).
