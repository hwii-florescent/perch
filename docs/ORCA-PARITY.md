...
|---|---|---|
| Worktree create: background with progress row, cancel, retry (`model/worktrees`) | ✅ `server/worktree_jobs.rs` (`worktree.job.*`): phase row, cancel undoes only what the job made, retry/dismiss. Local host only; remote hosts keep the synchronous `worktree.create`. The relay carries start-from, delete and branch review to a remote perch, but `host.info` has no capability list, so the web UI shows remote hosts none of the capability-gated workspace controls | `main/git`, `main/runtime` |
